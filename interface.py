...
class Interface:
    ...
    def __init__(self):
        ...
        """
        self.root = tk.Tk()
        self.root.title("Pesquisador de Egresso")
        ...
        self.search_button = tk.Button(self.root, text="Procurar", 
                                       command=self.open_search_page, width=20)
        self.search_button.pack(pady=10)
        
        self.connect_button = tk.Button(self.root, text="Conexão Automatica",
    ...
    def open_search_page(self):
        ...
        search_page.title("Procurar por Egressos")
        ...
        filters_frame = tk.LabelFrame(search_page, text="Filtros")
      ...
      # Criar o campo de entrada para "Universidade"
        university_label = tk.Label(filters_frame, text="Universidade:")
        university_label.pack(pady=5)
        university_entry = tk.Entry(filters_frame)
        university_entry.pack()
        try:
            university_entry.insert(0, saved_settings[2])  # Assumindo que o terceiro valor salvo seja a universidade
        except:
            pass

        # Create Advanced Configuration frame
        adv_config_frame = tk.LabelFrame(search_page, text="Avançado")
        adv_config_frame.pack(pady=10)

        # Create output location field
        output_label = tk.Label(adv_config_frame, text="Output:")
        ...
        # Create Search button
        search_button = tk.Button(search_page, text="Procurar", width=20,
                command=lambda: self.from_search_gui(position_entry.get(), location_entry.get(),
                                                    university_entry.get(),
                                                    output_entry.get(),
                                                    api_entry.get(),
                                                    repeat_var.get()))
    ...
    def open_connect_page(self):
        ...
        user_label = tk.Label(linkedin_frame, text="LinkedIn User:")
        ...
        password_label = tk.Label(linkedin_frame, text="LinkedIn Senha:")
        password_label.pack(pady=5)
        password_entry = tk.Entry(linkedin_frame, show="*")
        password_entry.pack(padx=6, pady=5)

        # Set filters frame
        filters_frame = tk.LabelFrame(connect_page, text="Filtro")
        filters_frame.pack(pady=10)

        # Connection message label and entry box
        con_msg_label = tk.Label(filters_frame, text="Mensagem para Conexão:")
        ...
        num_req_label = tk.Label(filters_frame, 
                                 text="Número de Request enviados:")
        ...
        # Set advanced frame
        adv_frame = tk.LabelFrame(connect_page, text="Avançado:")
        adv_frame.pack(pady=10)

        # Excel file path label and entry box
        exl_label = tk.Label(adv_frame, text="Excel path:")
        ...
        # Set Connect button
        search_button = tk.Button(connect_page, text="Conectar", width=20,
    ...
    def from_search_gui(self, positions, locations, 
                        university, output, api_key, repeat):
        """
        ...
        """
        self.save_search_settings(positions, locations, university, output, api_key, repeat)
        search = True
        try:
            if locations !="":
            ...
            if university != "":
                ...
                search = False
                self.search_message = "University must not be blank."
                self.display_search_message()
                raise ValueError
            ...
            if search:
                self.search_message="Procurando..."
                self.display_search_message()
    ...
    def save_search_settings(self, positions, locations, university, output, api_key, repeat):
        """
